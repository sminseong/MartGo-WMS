MartGO - WMS(창고 관리 시스템)

📌 프로젝트 개요

MartGO는 물류 창고(Warehouse Management System, WMS)의 프로세스를 학습하고 이해하는 것을 목표로 한 프로젝트입니다. Java와 MySQL을 활용하여 WMS의 핵심 기능을 구현하며, MVC 패턴을 적용하여 구조적인 개발을 경험하고 협업 및 Git을 활용한 프로젝트 관리 역량을 습득하는 것을 목적으로 합니다.
...
재고 관리, 재고 변경 이력, 산출물 관리

방민영

임대 관리, 용적률 계산, 창고 및 섹터 관리

서민성

입고 및 출고 프로세스, 제품 등록 시스템
...
📦 MartGO
 ...
 ┃ ┣ 📂 common            # 공통 기능
 ┃ ┃ ┣ 📂 config         # 설정 파일
 ┃ ┃ ┣ 📂 constants     # 상수 관리 (메시지, 오류 코드)
 ┃ ┃ ┣ 📂 exception     # 예외 처리 클래스
 ┃ ┃ ┣ 📂 utils         # 유틸리티 (입력 검증, 날짜 변환 등)
 ┃ ┣ 📂 controller        # 사용자 입력 및 요청 처리
 ┃ ┣ 📂 model            # 비즈니스 로직 및 데이터 계층
 ┃ ┃ ┣ 📂 dao           # 데이터 액세스 객체 (DB 연동)
 ┃ ┃ ┣ 📂 dto           # 데이터 전송 객체 (VO, DTO)
 ┃ ┃ ┣ 📂 service       # 서비스 레이어 (비즈니스 로직)
 ┃ ┣ 📂 view             # UI 또는 콘솔 출력 관련 클래스
 ┗ 📜 README.md
...
이 프로젝트는 신세계 I&C 6차수 3팀 전먹사에 의해 개발되었습니다. 🚀
